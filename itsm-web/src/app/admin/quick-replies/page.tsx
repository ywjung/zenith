  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-gray-900">빠른 답변 템플릿</h2>
          <p className="text-xs text-gray-500 mt-0.5">자주 사용하는 답변을 등록하면 코멘트 입력 시 빠르게 선택할 수 있습니다.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {showForm && (
        <div className="bg-white border rounded-lg shadow-sm p-5 space-y-4">
          <h3 className="text-sm font-semibold text-gray-800">{editing ? '빠른 답변 수정' : '새 빠른 답변'}</h3>
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={closeForm} className="text-sm px-4 py-2 border rounded-md text-gray-600 hover:bg-gray-50">취소</button>
            <button
              onClick={handleSave}
            ...
            <div key={item.id} className="bg-white border rounded-lg shadow-sm p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 whitespace-pre-wrap line-clamp-3">{item.content}</p>
                </div>
