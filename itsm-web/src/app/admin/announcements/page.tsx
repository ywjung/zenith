...
const TYPE_CONFIG = {
  info:     { label: '일반 정보', icon: 'ℹ️', bg: 'bg-blue-50',   border: 'border-blue-400',  text: 'text-blue-800',   badge: 'bg-blue-100 text-blue-700' },
  warning:  { label: '주의',     icon: '⚠️', bg: 'bg-yellow-50', border: 'border-yellow-400', text: 'text-yellow-800', badge: 'bg-yellow-100 text-yellow-700' },
  critical: { label: '긴급',     icon: '🚨', bg: 'bg-red-50',    border: 'border-red-500',   text: 'text-red-900',    badge: 'bg-red-100 text-red-700' },
}
...
export default function AnnouncementsPage() {
  ...
  async function toggleEnabled(ann: Announcement) {
    await fetch(`${API_BASE}/admin/announcements/${ann.id}`, {
      method: 'PUT',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-gray-900">공지사항 / 배너 관리</h2>
            ...
            <div className="flex gap-3 mt-3 text-sm">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gray-100 text-gray-600">
                전체 <strong className="text-gray-900">{list.length}개</strong>
              </span>
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full ${activeCount > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-blue-200 shadow-sm">
          ...
          <div className="p-6 space-y-5">
            {/* 유형 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">배너 유형</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  만료 일시 <span className="text-gray-400 font-normal">(비우면 무기한)</span>
                </label>
                <input
                  type="datetime-local"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">노출 상태</label>
                <button
                  type="button"
            ...
            {/* 미리보기 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">배너 미리보기</label>
              <div className="border border-dashed border-gray-300 rounded-xl p-3 bg-gray-50">
                ...
                {!form.title && !form.content && (
                  <p className="text-xs text-gray-400 text-center py-2">제목을 입력하면 미리보기가 표시됩니다</p>
                )}
              </div>
            </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {list.map(ann => {
                ...
                return (
                  <tr key={ann.id} className={`hover:bg-gray-50 transition-colors ${!ann.enabled || isExpired ? 'opacity-50' : ''}`}>
                    {/* 유형 */}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        <h4 className="text-sm font-semibold text-amber-800 mb-3">📌 공지사항 운영 가이드</h4>
