...
const PRIORITY_META: Record<string, {
  ...
  critical: { label: '긴급', icon: '🔴', color: 'text-red-700',    bg: 'bg-red-50',    border: 'border-red-200',    bar: 'bg-red-400',    desc: '업무 전체 중단 · 즉시 대응' },
  high:     { label: '높음', icon: '🟠', color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200', bar: 'bg-orange-400', desc: '주요 기능 장애 · 빠른 처리 필요' },
  medium:   { label: '보통', icon: '🟡', color: 'text-yellow-700', bg: 'bg-yellow-50', border: 'border-yellow-200', bar: 'bg-yellow-400', desc: '일부 불편 · 업무 지속 가능' },
  low:      { label: '낮음', icon: '⚪', color: 'text-gray-600',   bg: 'bg-gray-50',   border: 'border-gray-200',   bar: 'bg-gray-400',   desc: '사소한 개선 요청' },
}
            ...
            return (
              <div key={policy.priority} className={`rounded-xl border ${meta.border} ${meta.bg} p-5 flex flex-col gap-4`}>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {!isEditing && (
                      <button
                        ...
                        className="text-xs text-gray-400 hover:text-gray-700 px-2 py-0.5 rounded hover:bg-white/60"
                      >
                        수정
                      </button>
                    )}
                  ...
                  <div className="text-xs text-gray-500 mt-0.5 leading-snug">{meta.desc}</div>
                </div>

                {isEditing ? (
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs text-gray-500 font-medium block mb-1">최초 응답 (시간)</label>
                      <input
                        type="number"
                        min={1}
                        max={999}
                    ...
                    <div>
                      <label className="text-xs text-gray-500 font-medium block mb-1">해결 시간 (시간)</label>
                      <input
                        type="number"
                        min={1}
                        max={999}
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between text-xs mb-1.5">
                        <span className="text-gray-500">해결 시간</span>
                    ...
                    {(policy.updated_by || policy.updated_at) && (
                      <div className="text-[10px] text-gray-400 pt-1 border-t border-white/60">
