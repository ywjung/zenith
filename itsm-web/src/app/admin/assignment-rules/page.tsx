...
function RuleFormPanel({
  ...
  return (
    <form onSubmit={onSubmit} className="bg-white border border-blue-200 rounded-xl p-5 mb-2 shadow-sm">
      <h3 className="font-semibold text-gray-800 mb-4">{title}</h3>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">규칙 이름 *</label>
          <input
            value={form.name}
        ...
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">우선순위 순서 (높을수록 먼저)</label>
          <input
            type="number"
            value={form.priority}
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-4">
        <div className="text-xs font-semibold text-gray-500 mb-3">조건 (비워두면 전체 티켓에 적용)</div>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            <label className="text-xs text-gray-600 block mb-1">우선순위</label>
            <select
          ...
          <div>
            <label className="text-xs text-gray-600 block mb-1">제목 키워드</label>
            <input
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">담당자 GitLab ID *</label>
          <input
            type="number"
        ...
        <div>
          <label className="text-xs font-medium text-gray-600 block mb-1">담당자 이름 *</label>
          <input
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onCancel} className="border border-gray-300 px-5 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50">취소</button>
      </div>
    </form>
  )
}
          ...
          {rules.map((rule) => (
            <div key={rule.id}>
              {editingId === rule.id ? (
                ...
                <div
                  className={`bg-white rounded-xl border px-5 py-4 flex items-center gap-4 transition-opacity ${
                ...
                >
                  ...
                  <div className="w-40 flex-shrink-0">
                    <div className="font-medium text-gray-900 text-sm truncate">{rule.name}</div>
                  </div>

                  {/* IF conditions */}
                  <div className="flex-1 flex items-center gap-1.5 flex-wrap">
                    <span className="text-xs text-gray-400 font-mono font-semibold">IF</span>
                    {rule.match_category && (
                    ...
                    {!rule.match_category && !rule.match_priority && !rule.match_keyword && (
                      <span className="text-xs text-gray-400 italic">모든 티켓</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span className="text-sm text-gray-700">{formatName(rule.assignee_name)}</span>
                  </div>
